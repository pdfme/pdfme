...
### 1. `md2pdf` default style 品質改善

- PR #1485 で進行中。MVP の「作れる」状態から「自然に読める」状態へ近づける。
- まずは API を増やさず、既存 `style` overrides を保ったまま default の見た目を整える。
- heading scale / heading margin、paragraph spacing、lineHeight、list spacing、table grid、code block
  background + padding、blockquote left rule + padding、horizontal rule color を整える。
- text / MVT の `padding`, `borderWidth`, `borderColor` を追加し、table cell と同じ box helper を使う。
  issue #851 の要求をここで小さく回収する。
- `style?: { preset?: "default"; ...overrides }` のような preset API は、複数 preset が必要になった時に
  追加検討する。full CSS parser にはしない。
...
- code block の等幅フォント、長い行、syntax highlight は layout 品質フォローアップで扱う。
