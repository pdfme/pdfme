...
const imageSchema: Plugin<ImageSchema> = {
  pdf: async (arg) => {
    ...
    if (fitLayout.objectFit === 'cover') {
      ...
      const { x: clipX, y: clipY } = clippingBox.position;
      page.pushOperators(
        pdfLib.pushGraphicsState(),
        pdfLib.moveTo(clipX, clipY),
        pdfLib.lineTo(clipX, clipY + clippingBox.height),
        pdfLib.lineTo(clipX + clippingBox.width, clipY + clippingBox.height),
        pdfLib.lineTo(clipX + clippingBox.width, clipY),
        pdfLib.closePath(),
        pdfLib.clip(),
        pdfLib.endPath(),
      );
