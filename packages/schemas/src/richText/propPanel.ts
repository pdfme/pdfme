...
import type { RichTextLetterStyle, RichTextSchema } from './types';
import {
  DEFAULT_FONT_NAME,
  DEFAULT_FONT_SIZE,
  DEFAULT_ALIGNMENT,
...
const constructContent = (content: string, style: RichTextLetterStyle) => {
  return JSON.stringify(content.split('').map((letter) => ({ letter, style })));
};
...
export const propPanel: PropPanel<RichTextSchema> = {
  schema: ({ options, activeSchema, i18n }) => {
    ...
    const RichTextSchema: Record<string, PropPanelSchema> = {
      fontName: {
  ...
  defaultSchema: {
    name: '',
    type: 'richText',
    content: constructContent('RichText', {
