...
describe('changeSchemas test', () => {
  ...
  test('getDynamicHeightReflowChanges - list auto height pushes lower schemas down', () => {
    const listSchema = {
      id: uuid(),
      ...getSchema(),
      name: 'items',
  ...
  test('getDynamicHeightReflowChanges - ignores non-dynamic schemas', () => {
    const textSchema = {
      id: uuid(),
      ...getSchema(),
      name: 'items',
    ...
    expect(
      getDynamicHeightReflowChanges({
        schemas: [textSchema, belowSchema],
        schema: textSchema,
        height: 25,
      }),
    ).toStrictEqual([]);
