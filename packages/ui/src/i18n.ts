...
const dictEn: { [key in keyof Dict]: string } = {
  ...
  availableDynamicFields: 'Available Dynamic Fields',
...
const dictJa: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: '使用済み',
  availableDynamicFields: '使用可能な動的フィールド',
...
const dictKo: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: '이미 사용됨',
  availableDynamicFields: '사용 가능한 동적 필드',
...
const dictAr: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: 'مستخدم',
  availableDynamicFields: 'حقول ديناميكية متاحة',
...
const dictTh: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: 'มีการใช้งานแล้ว',
  availableDynamicFields: 'ฟิลด์ที่สามารถใช้งานได้',
...
const dictIt: { [key in keyof Dict]: string } = {
  ...
  availableDynamicFields: 'Campi Dinamici Disponibili',
...
const dictPl: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: 'Używane',
  availableDynamicFields: 'Dostępne pola dynamiczne',
...
const dictDe: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: 'Verwendet',
  availableDynamicFields: 'Verfügbare dynamische Felder',
...
const dictEs: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: 'Usado',
  availableDynamicFields: 'Campos dinámicos disponibles',
...
const dictFr: { [key in keyof Dict]: string } = {
  ...
  alreadyUsed: 'Utilisé',
  availableDynamicFields: 'Champs dynamiques disponibles',
