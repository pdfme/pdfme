...
import { Plugin, Schema, Mode } from '@pdfme/common';
import { convertForPdfLayoutProps } from '@pdfme/schemas';

// TMP
const isEditable = (mode: Mode) => mode === 'form' || mode === 'designer';
...
export const svg: Plugin<SVG> = {
  ui: (arg) => {
    ...
    if (isEditable(mode)) {
      ...
      const svgElement = container.childNodes[0] as SVGElement;
  ...
  pdf: async (arg) => {
    ...
    // TODO ここで xmldomでsvgをパースしてvalidateする
    /*
